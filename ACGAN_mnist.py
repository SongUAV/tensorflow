...
def discriminator():
	...
	return Model(input=image,output=[prob,digit])
...
def generator(latent_size):
	...
	return Model(input=[latent,image_digit],output=fake_image)
...
D=discriminator()
D.compile(optimizer=Adam(lr=adam_lr*2,beta_1=adma_beta_1),loss=['binary_crossentropy','sparse_categorical_crossentropy'])

G=generator(latent_size)
#G.compile(optimizer=Adam(lr=adam_lr*10,beta_1=adma_beta_1),loss='binary_crossentropy')

latent=Input(shape=(latent_size,))
image_class=Input(shape=(1,),dtype='int32')

fake_image=G([latent,image_class])

D.trainable=False
prob,digit=D(fake_image)

AD=Model(input=[latent,image_class],output=[prob,digit])
AD.compile(optimizer=Adam(lr=adam_lr,beta_1=adma_beta_1),loss=['binary_crossentropy','sparse_categorical_crossentropy'])
...
for epoch in range(nb_epochs):
	...
	for index in range(nb_batches):
		...
		epoch_gen_loss.append(AD.train_on_batch([noise,sampled_labels.reshape((-1,1))],[y,sampled_labels]))
	#print ('\nTesting for epoch{}:'.format(epoch+1))

	noise=np.random.uniform(-1,1,(nb_test,latent_size))
	sampled_labels=np.random.randint(0,10,nb_test)
	generated_images=G.predict([noise,sampled_labels.reshape((-1,1))],verbose=False)

	x=np.concatenate((x_test,generated_images))
	y=np.array([1]*nb_test+[0]*nb_test)
	aux_y=np.concatenate((y_test,sampled_labels),axis=0)

	discriminator_test_loss=D.evaluate(x,[y,aux_y],verbose=False)
	...
	y=np.ones(2*nb_test)
	generator_test_loss=AD.evaluate([noise,sampled_labels.reshape((-1,1))],[y,sampled_labels],verbose=False)
	generator_train_loss=np.mean(np.array(epoch_gen_loss),axis=0)

	print ("\n generator_test_loss is %f " % generator_test_loss[-1])
	print ("\n generator_train_loss is %f " % generator_train_loss[-1])
	print ("\n discriminator_test_loss is %f " % discriminator_test_loss[-1])
	print ("\n discriminator_train_loss is %f " % discriminator_train_loss[-1])
